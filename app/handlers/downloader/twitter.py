...
from app.services.twitter_services import download_twitter
...
async def tw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            ...
            "<code>/tw https://twitter.com/user/status/xxx</code>",
            parse_mode="HTML"
        )
        return
    ...
    if not any(domain in url for domain in ["twitter.com", "x.com", "t.co"]):
        await update.message.reply_text("❌ Link tidak valid. Gunakan link dari Twitter/X.")
        return
    ...
    msg       = await update.message.reply_text("📥 Downloading Twitter/X...")
    file_path = None

    try:
        file_path = await asyncio.to_thread(download_twitter, url)
